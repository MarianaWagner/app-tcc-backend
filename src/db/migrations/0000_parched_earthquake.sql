...
CREATE TABLE "share_link" (
	...
	"created_at" timestamp with time zone DEFAULT now(),
	"updated_at" timestamp with time zone DEFAULT now(),
	CONSTRAINT "share_link_code_unique" UNIQUE("code")
);
--> statement-breakpoint
...
CREATE TABLE "user" (
	...
	"term_accepted" timestamp with time zone,
	"term_version" text,
	"created_at" timestamp with time zone DEFAULT now(),
	"updated_at" timestamp with time zone DEFAULT now(),
	"deleted_at" timestamp with time zone
...
ALTER TABLE "shared_exam" ADD CONSTRAINT "shared_exam_exam_id_exam_id_fk" FOREIGN KEY ("exam_id") REFERENCES "public"."exam"("id") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
